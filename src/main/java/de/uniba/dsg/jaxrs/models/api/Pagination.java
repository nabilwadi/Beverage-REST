...
public class Pagination {
    ...
    public Pagination(
            final URI path, final int currentPage, final int pageLimit, final int collectionSize) {
        this.page = currentPage;
        this.noOfPages = (int) Math.ceil((double) collectionSize / (double) pageLimit);

        this.first =
                UriBuilder.fromUri(path)
                        .queryParam("page", 1)
                        .queryParam("pageLimit", pageLimit)
                        .build();
        this.last =
                UriBuilder.fromUri(path)
                        .queryParam("page", this.noOfPages)
                        .queryParam("pageLimit", pageLimit)
                        .build();
        this.previous =
                UriBuilder.fromUri(path)
                        .queryParam("page", Math.max((this.page - 1), 1))
                        .queryParam("pageLimit", pageLimit)
                        .build();
        this.next =
                UriBuilder.fromUri(path)
                        .queryParam("page", Math.min((this.page + 1), this.noOfPages))
                        .queryParam("pageLimit", pageLimit)
                        .build();
    }
